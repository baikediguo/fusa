...
# 配置区：指定文件名、参数、门列表等
VERILOG_FILE = 'pe.synth_dct.v'     # 被注入的门级Verilog文件
TB_FILE = 'tb.v'                    # 测试平台文件
CELLS_FILE = 'cells.v'              # 标准单元库文件
...
def run_simulation(fault_v, tb_v, cells_v, log_path):
    ...
    """
    exec_cmd = f"iverilog -o simv_{fault_v.replace('.v','')} {tb_v} {fault_v} {cells_v} && vvp simv_{fault_v.replace('.v','')}"
    ...
    for idx, (inst_name, module_type, inst, mod) in enumerate(instances):
        ...
        for out_port in outs:
            for stuck in STUCK_VALUES:
                ...
                fault_v = f'fault_{inst_name}_{out_port}_sa{stuck}.v'
                log_path = f'fault_{inst_name}_{out_port}_sa{stuck}.log'
                if ok:
